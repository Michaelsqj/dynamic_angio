function [time, gA, kA, p, fact] = base_cone6(mat, fov, Ts, gmax, smax, readout_time, cone_angle, varargin)
...
%     readout_time = readout_time - Ts;
    ...
    for step = [1 0.5 0.1 0.05 0.01 0.005]
        ...
        while rtime < readout_time
            fact = fact + step;
            k = genkT(fact, p, n, midpt, kmax, cone_angle);
...
function k = genkT(fact, p, n, midpt, kmax, cone_angle)
    ...
    kTr2 = t2.^n * kmax * sin(cone_angle / 180 * pi);
    ...
    % kTr = t.^n * kmax * sin(cone_angle / 180 * pi);

    % midpt = 700;
    % kTr1 = (k(1:midpt, 3).^n / k(midpt, 3).^n) * sqrt( kmax^2 - k(midpt,3).^2 );
    % kTr2 = sqrt( kmax^2 - k(midpt+1:end,3).^2 );
    kTr = [kTr1(:); kTr2(:)]';
